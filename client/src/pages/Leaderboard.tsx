              ...
              {users?.map((user, index) => {
                ...
                return (
                  <Link href={`/profile/${user.id}`} key={user.id}>
                    <div className={`grid grid-cols-12 gap-4 p-4 items-center cursor-pointer transition-all duration-300 hover:bg-white/5 ${index === 0 ? "bg-primary/5 border-l-2 border-l-primary" : ""
                      }`}>
                      ...
                      <div className="col-span-7 md:col-span-8 flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full overflow-hidden border-2 ${isTop3 ? "border-primary" : "border-transparent bg-muted"
                          }`}>
                      ...
                      <div className="col-span-3 text-right pr-4">
                        <span className={`font-serif text-xl font-bold ${isTop3 ? "text-primary drop-shadow-[0_0_5px_rgba(212,175,55,0.4)]" : "text-muted-foreground"
                          }`}>
                          {user.points || 0}
                        </span>
                      </div>
